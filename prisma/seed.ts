...
function dt(date: string, time = "08:00"): Date {
...
async function main() {
  ...
  // ========== CLIENTS (12) - 8 individuales + 4 empresas ==========
  ...
  const itm = (sku: string, name: string, desc: string, cat: any, sup: any, unit: string, cost: number, price: number, stock: number, minS: number, maxS: number, loc: string, imp?: boolean, customs?: number) => prisma.inventoryItem.upsert({
    where: { sku },
  ...
  const we = (oid: string, ev: string, desc: string, uid: string, created: Date) => ({ workOrderId: oid, event: ev, description: desc, userId: uid, createdAt: created })
  const events = [
  ...
  if (evCount === 0) { await prisma.workOrderEvent.createMany({ data: events }); console.log("  OK " + events.length + " eventos") }
  else { console.log("  Eventos ya existentes, omitiendo") }
  // ========== ATTENDANCE (últimos 30 días, todo el personal) ==========
  ...
  const set = (key: string, value: string, cat: string) => prisma.setting.upsert({
    where: { key },
  ...
  } else { console.log("  Notificaciones ya existentes, omitiendo") }
